...
class FeedbackWeight:
    ...
    def take_result_percent(self):
        weight = (1, 2, 3, 4)  # 权重得分：假设观看得0.3分，点赞得1分，评论得2分，分享得3分，投币得4分
        # weight = (1, 2, 3)
        base_cut = 0.05  # 将观看数规模缩减后进行比较
        return (self.take_like() * weight[0] + self.take_comment() * weight[1] + self.take_share() * weight[2] + self.take_coin() * weight[3]) / (self.take_watch() * base_cut)
        # return (self.take_like() * weight[0] + self.take_comment() * weight[1] + self.take_share() * weight[2]) / (self.take_watch() * base_cut)
